...
class RNN_network(nn.Module):
    def __init__(self, input_size=64, hidden_size=64, num_layers=2, num_embedding=20, use_gpu=False, dropout=0.8):
        ...
        self.hidden_size = hidden_size  # LSTM 层的隐藏状态的维度
        ...
        self.fc = nn.Linear(hidden_size, num_embedding)  # project to 2-label data  #全连接层，将 LSTM 的输出特征投影到 2-label 数据空间，输出维度为20
        # self.dropout_1 = nn.Dropout(dropout)
        # self.dropout_2 = nn.Dropout(dropout)
        self.h0 = None  #h0 -> init  # hidden state
        self.c0 = None  #c0 -> init  # cell state
...
class Basic_CNN(nn.Module):
    def __init__(self, base_network='vgg_16', pretrained=False, rnn_input_size=64, rnn_hidden_size=64,
                 rnn_num_layers=1, num_classes=3, num_embedding = 20, use_gpu=False, frozen_weights = False, 
                 dropout_CNN=0.5, dropout_LSTM=0.8, video_length = 8):  #classes -> slip or not
        super(Basic_CNN, self).__init__()

        # return features_pinching, features_sliding # (64,1) tensor for LSTM model
        self.cnn_network = Basic_network(base_network=base_network, pretrained=pretrained, frozen_weights=frozen_weights, 
                                         dropout=dropout_CNN, rnn_input_size = rnn_input_size)
        # 创建了 ResNet18 网络，经过两个线性层后，第一个线性层输入大小为 4096*2(两个 ResNet18 输出的特征拼接而成)，第二个线性层输出为 rnn_input_size = 64 
    ...
    def forward(self, x1, x2, y1, y2, thresh):
        ...
        for i in range(self.video_length):   # get 8 * (64, 1) tensor (a sequence and then fed into LSTM)
            ...
            if i == 0:
                cnn_features_pinching = features_pinching.unsqueeze(1)  #在第二个维度上添加一个新的维度，将原来形状 (batch_size,...) 张量变形为 (batch_size, 1,...)
                cnn_features_sliding = features_sliding.unsqueeze(1)
            else:
                cnn_features_pinching = torch.cat([cnn_features_pinching, features_pinching.unsqueeze(1)], dim=1)  #将两个张量按照维度 1 (第二个维度进行拼接)
        ...
        #两个张量沿着最后一个维度(-1)进行拼接，这两个张量的形状是 (batch_size,num_embedding),即 (4,20),拼接后变成 (4,40)
