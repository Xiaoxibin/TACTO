...
def get_model(config_data,use_gpu):
    ...
    video_length = config_data['video_length']   # 8
    ...
    if model_type == 'vivit_fdp_two':
        ...
        Model = vivit_FDP_two.VIVIT(Tactile_img_size=Tactile_img_size, Visual_img_size = Visual_img_size, Tactile_patch_size=Tactile_patch_size, 
                                    ...
                                    attn_drop_rate=attn_dropout, drop_path_rate=dropout, num_frames=num_frames, dropout=0.,use_gpu=use_gpu)

        model_params = {'Tactile_img_size':Tactile_img_size, 'Visual_img_size':Visual_img_size, 'Tactile_patch_size':Tactile_patch_size, 
                        'Visual_patch_size':Visual_patch_size, 'in_chans':in_chans,'num_cls': num_cls, 'emb_dim':emb_dim, 'depth':depth, 
                        'num_heads':num_heads,'mlp_ratio':mlp_ratio, 'dropout':dropout, 'attn_drop_rate':attn_dropout, 
                        'num_frames':num_frames}
        
    elif model_type == 'basic_CNN':  # the benchmark for comparison
        # The bacis CNN + LSTM (the same one in slip detection paper)
        base_network = config_data['base_network']  #ResNet18
        pretrained_ = config_data['pretrained']  #False
        frozen_weights = config_data['frozen_weights']  # False -> all parameters are learnable   #False
        ...
        Model = timeSformer_orig_two.TimeSFormer(Tactile_img_size=Tactile_img_size, Visual_img_size = Visual_img_size, 
                                                 Tactile_patch_size=Tactile_patch_size, Visual_patch_size=Visual_patch_size, 
                                                 in_chans=in_chans,num_classes=num_cls, embed_dim=emb_dim, depth=depth, num_heads=num_heads, 
                                                 mlp_ratio=mlp_ratio, qkv_bias=False, qk_scale=None, drop_rate=dropout, attn_drop_rate=attn_dropout, 
                                                 drop_path_rate=dropout,hybrid_backbone=None, norm_layer=nn.LayerNorm, num_frames=num_frames, 
                                                 attention_type='divided_space_time', dropout=0.,use_gpu=use_gpu)
